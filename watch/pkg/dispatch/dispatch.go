...
type Manager struct {
	...
	enabled           bool
	workers           int
	filter            *beewatch.EventFilter
}

type DispatchOption func(*Manager)

func WithEventFilter(filter *beewatch.EventFilter) DispatchOption {
	return func(m *Manager) { m.filter = filter }
}
...
func New(cfg Config, log *zap.Logger, attachToServer *grpc.Server, opts ...DispatchOption) (*Manager, error) {
	...
	if m.filter != nil {
		m.eventSubscriber, err = subscriber.NewService(log, cfg.CheckpointFrequency, m.wg, attachToServer, diskStore, subscriber.WithEventFilter(m.filter))
	} else {
		m.eventSubscriber, err = subscriber.NewService(log, cfg.CheckpointFrequency, m.wg, attachToServer, diskStore)
	}
	if err != nil {
		return nil, err
	}
