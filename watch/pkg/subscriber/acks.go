...
type Checkpointer interface {
	Store(metaID uint32, seqID uint64)
	Flush() error
...
var _ Checkpointer = &DiskStore{}

// Store updates the in-memory checkpoint for metaID to seqID. Call Flush to persist to disk.
func (d *DiskStore) Store(metaID uint32, seqID uint64) {
	d.mu.Lock()
	defer d.mu.Unlock()
...
// Flush atomically flushes all checkpoints to disk. It must be called from a single goroutine to
// prevent races.
