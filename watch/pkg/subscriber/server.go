...
func NewServer(log *zap.Logger, config Config, checkpoint Checkpointer, opts ...ServiceOption) (*Server, error) {
	...
	svc, err := NewService(log, config.AckFrequency, grpcServer, checkpoint, opts...)
	if err != nil {
		return nil, err
	}
...
func (s *Server) ListenAndServe(events chan<- *bw.Event, acks <-chan Ack, errChan chan<- error) {
	...
	go func() {
		...
		if err != nil {
			s.reportFatal(errChan, fmt.Errorf("subscriber: error listening on address %s: %w", s.config.Address, err))
			return
		}
		s.log.Info("serving gRPC requests")
		if err := s.grpcServer.Serve(lis); err != nil {
			s.reportFatal(errChan, fmt.Errorf("subscriber: error serving gRPC requests: %w", err))
		}
	}()
}

// Stop shuts down the dedicated gRPC server then waits for all in-progress ReceiveEvents handlers
// to finish. It uses grpcServer.Stop() (not GracefulStop) so that blocked stream.Recv() calls are
// cancelled immediately, allowing handlers to exit cleanly.
//
// Stop does not close the acks channel or wait for the final checkpoint flush; the owner of acks
// closes it (after its producers stop) and may call WaitFlushed to confirm the flush completed.
func (s *Server) Stop() {
...
// reportFatal hands a fatal server error to errChan, or logs it if no receiver is listening (e.g.
// shutdown already in progress) so the goroutine never blocks.
func (s *Server) reportFatal(errChan chan<- error, err error) {
	select {
	case errChan <- err:
	default:
		s.log.Error("subscriber server error with no errChan receiver", zap.Error(err))
	}
}
