...
func (m *Manager) GetEventDispatchFunc(ctx context.Context, log *zap.Logger) dispatch.DispatchFunc {
	return func(event *beewatch.Event) {
		...
		log.Info("detected a blocked open event, checking if the stub file is eligible for automatic restore",
		...
		if err != nil {
			log.Warn("skipping automatic restore as the stub file's contents could not be retrieved", zap.String("path", path), zap.Error(err))
			return
		}
		...
		if err != nil {
			log.Warn("unable to create sync job to restore the specified stub file's contents", zap.String("path", path), zap.Error(err))
			return
		}

		log.Info("stub file restore triggered", zap.Any("result", result))
	}
}
