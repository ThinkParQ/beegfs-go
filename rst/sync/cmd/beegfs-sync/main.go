...
func main() {
	...
	pflag.Int64("manager.journal-db-block-cache", 256<<20, "Size in bytes of the in-memory block cache used to speed up reads from the work journal DB. Increase this on nodes handling a large volume of work requests to avoid excessive disk reads.")
	...
	pflag.Int64("manager.job-db-block-cache", 256<<20, "Size in bytes of the in-memory block cache used to speed up reads from the job store DB. Increase this on nodes handling a large volume of work requests to avoid excessive disk reads.")
...
`
	...
	// Logger must be shut down last; the OTel exporter's own timeout bounds the flush.
	defer func() {
