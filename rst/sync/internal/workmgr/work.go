...
func (w *worker) processWork(work workAssignment) {
	...
	if request.JobBuilder {
		...
		g.Go(func() error {
			return client.ExecuteJobBuilderRequest(ctx, request.WorkRequest, jobSubmissionChan)
		})
	...
	processJobs:
		for {
			...
			case <-ctx.Done():
				...
				status.SetMessage("not all jobs were submitted: " + ctx.Err().Error())
			case jobRequest, ok := <-jobSubmissionChan:
				...
				if err := w.beeRemoteClient.SubmitJobRequest(ctx, jobRequest); err != nil {
					status.SetState(flex.Work_CANCELLED)
					status.SetMessage("not all jobs were submitted: " + err.Error())
					break processJobs
				}
