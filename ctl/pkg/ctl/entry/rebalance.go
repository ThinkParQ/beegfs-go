...
// specified srcMirrors or srcTargets to dstPool. It returns true if at least one job was started
// (an error might also be returned), or if dryRun was set and at least once mirror/target needs
// rebalancing for this entry.
//
// IMPORTANT: dstMirrors and dstTargets should not contain any duplicates but must be provided as
...
func startRebalancingJobs(
	...
	nextCandidate:
		for ; shuffledIdx < len(shuffledIDs); shuffledIdx++ {
			...
			return shuffledIDs[shuffledIdx], nil
		}
