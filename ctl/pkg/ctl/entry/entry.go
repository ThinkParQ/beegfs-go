...
func SetFileRstIds(ctx context.Context, path string, rstIds []uint32) error {
	entry, ownerNode, err := getEntryAndOwnerFromPathWithCached(ctx, path)
	if err != nil {
		return err
	}
...
// getEntryAndOwnerFromPathWithCached returns the results from GetEntryAndOwnerFromPath() using the
// cached mappings. If there's a failure for any reason a subsequent attempt will be made after
// updating the cached mappings.
func getEntryAndOwnerFromPathWithCached(ctx context.Context, path string) (msg.EntryInfo, beegfs.Node, error) {
