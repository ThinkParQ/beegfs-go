	...
	if cfg.execute {
		...
		if err != nil {
			...
			if res.LegacyId.NodeType == beegfs.Meta {
				cmdfmt.Printf("Deleting a metadata node might cause adverse effects to your filesystem and should only be done if no files using it or its internal target are left. If you really want to delete the node, please add the --yes flag to the command. ")
			} else {
