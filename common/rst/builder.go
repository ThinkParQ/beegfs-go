...
func (c *JobBuilderClient) getBuilderResults(builder *flex.BuilderJob) (err error) {
	...
	totalUpdateErrors := builder.GetUpdateDirErrors()
	...
	if totalUpdateErrors > 0 {
		err = appendError(err, fmt.Errorf("%d file or directory configuration updates failed to complete", totalUpdateErrors))
	}
...
func (c *JobBuilderClient) newRequestBuildController(
	...
	requestBuilder := c.newJobRequestBuilder(builderCfg, jobSubmissionCh, addToBulkRequest, jobRequestCountsCh)
...
func (c *JobBuilderClient) newJobRequestBuilder(
	...
	requestBuilder.initHandleDirRstConfig()
