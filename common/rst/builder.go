...
func (c *JobBuilderClient) ExecuteJobBuilderRequest(
	...
	controller, err := c.newRequestBuildController(shutdownCtx, workCtx, log, cfg, submitRequest, registry.AddRequest, registry.UpdateBulkRequest, workerSaturation)
	if err != nil {
		return &SchedulingResult{Err: err}
	}
...
func (c *JobBuilderClient) newRequestBuildController(
	...
	workerSaturation []func() float64,
) (*requestBuildController, error) {
	getPaths, err := c.getPathsFn(builderCfg)
	if err != nil {
		return nil, err
	}

	maxWorkers := max(1, int(requestBuildControllerWorkerMultiplier*float32(runtime.GOMAXPROCS(0))))
	requestBuilder := c.newJobRequestBuilder(log, builderCfg, submitRequest, addBulkRequest, updateBulkRequest)
...
func (c *JobBuilderClient) getPathsFn(cfg *flex.JobRequestCfg) (requestPathResolverFn, error) {
	if cfg.Download {
		...
		isPathDir := false
		if stat, err := c.mountPoint.Lstat(cfg.Path); err == nil {
		...
		return func(walkPath string) (string, string, error) {
			...
			return inMountPath, remotePath, nil
		}, nil
	}
