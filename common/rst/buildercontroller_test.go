...
// newFailingTestSubmitter returns a submitter that refuses every submission with err, standing in
// for remote being unreachable or rejecting the request outright.
func newFailingTestSubmitter(err error) *testSubmitter {
	return &testSubmitter{result: func(*beeremote.JobRequest) error { return err }}
}
