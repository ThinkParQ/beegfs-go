...
func prepareJobRequests(ctx context.Context, cfg *flex.JobRequestCfg) ([]*beeremote.JobRequest, error) {
	...
	// SkipPrepareJob=true to avoid making a request for remote file/object information which will
	// need to be handled by the rst client.
	requests, err := BuildJobRequests(ctx, rstMap, mountPoint, store, mappings, pathInfo.Path, cfg.RemotePath, cfg, true)
	if err != nil {
		return nil, fmt.Errorf("unable to build job requests: %w", err)
	}
	return requests, nil
}
