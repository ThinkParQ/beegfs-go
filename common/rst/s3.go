...
func (r *S3Client) GenerateWorkRequests(ctx context.Context, lastJob *beeremote.Job, job *beeremote.Job, availableWorkers int) (requests []*flex.WorkRequest, err error) {
	...
	if err := r.prepareJobRequest(ctx, r.getJobRequestCfg(request), sync); err != nil {
		return nil, err
	}
...
func (r *S3Client) completeSyncWorkRequests_Download(ctx context.Context, job *beeremote.Job, workResults []*flex.Work, abort bool) error {
	...
	if !abort {
		...
		if !request.StubLocal && IsFileOffloaded(sync.LockedInfo) {
			...
			entry.ClearFileDataState(ctx, mappings, store, request.Path)
		}
...
func (r *S3Client) prepareJobRequest(ctx context.Context, cfg *flex.JobRequestCfg, sync *flex.SyncJob) error {
	lockedInfo := sync.LockedInfo
	...
	if !IsFileLocked(lockedInfo) {
		if lockedInfo, _, err = GetLockedInfo(ctx, r.mountPoint, store, mappings, cfg, cfg.Path); err != nil {
			return err
		}
	...
	if !HasRemotePathInfo(lockedInfo) {
		request := PrepareAndBuildJobRequest(ctx, r, r.mountPoint, store, mappings, cfg)
		...
		if status != nil {
			...
			case beeremote.JobRequest_GenerationStatus_FAILED_PRECONDITION:
				return fmt.Errorf("%w: %s", ErrJobFailedPrecondition, status.Message)
			case beeremote.JobRequest_GenerationStatus_ERROR:
				return fmt.Errorf(status.Message)
			default:
				return fmt.Errorf("failed to prepare job request for an unknown reason: %s", status.Message)
			}
		}
