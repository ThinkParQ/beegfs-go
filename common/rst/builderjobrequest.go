...
type handleDirRstConfigFn func(ctx context.Context, inMountPath string) (isDir bool, updateErr bool, err error)
...
type jobRequestBuilder struct {
	...
	handleDirRstConfig handleDirRstConfigFn
}

type jobRequestCounts struct {
	Submitted       int32 // Jobs submitted
	Errors          int32 // Jobs submitted with errors
	Conflicts       int32 // Jobs that conflict with existing jobs
	UpdateDirErrors int32 // Update directory configuration errors
}

// initHandleDirRstConfig initializes handleDirRstConfig, pre-computing rstIds and cooldownSecs
// once so they are not recomputed for each path.
func (w *jobRequestBuilder) initHandleDirRstConfig() {
	if !(w.builderCfg.GetUpdate() || w.builderCfg.HasCooldownSecs()) {
		...
		w.handleDirRstConfig = func(context.Context, string) (bool, bool, error) { return false, false, nil }
		return
	}
	...
	w.handleDirRstConfig = func(ctx context.Context, inMountPath string) (bool, bool, error) {
		...
		if stat.IsDir() {
			updateErr := entry.SetDirRstPattern(ctx, inMountPath, rstIds, cooldownSecs)
			return true, updateErr != nil, nil
		}
...
func (w *jobRequestBuilder) Process(ctx context.Context, inMountPath string, remotePath string, failedPrecondition error) (err error) {
	...
	if isDir, updateErr, err := w.handleDirRstConfig(ctx, inMountPath); isDir {
		...
		if updateErr {
			result.UpdateDirErrors++
		}
		return nil
	} else if err != nil {
		// A missing path is silently skipped; any other stat failure is likely systemic and
		// terminates the builder job since GetPathState would hit the same error for any path type.
