...
type fakeBulkOperation struct {
	...
	updateMu        sync.Mutex
	updatedRequests []fakeBulkRequestUpdate
}
...
type fakeBulkRequestUpdate struct {
	request *beeremote.JobRequest
	state   BulkRequestState
}

func (f *fakeBulkOperation) UpdateBulkRequest(ctx context.Context, request *beeremote.JobRequest, state BulkRequestState) error {
	f.updateMu.Lock()
	defer f.updateMu.Unlock()
	f.updatedRequests = append(f.updatedRequests, fakeBulkRequestUpdate{request: request, state: state})
	return f.updateBulkRequestErr
}
...
// TestBulkOperationRegistry_UpdateBulkRequest asserts the registry routes a request's state back to
// the operation that owns it. The request only carries its operation name and rst id, so getting the
// lookup wrong would credit the transition to the wrong operation, or silently to none at all.
func TestBulkOperationRegistry_UpdateBulkRequest(t *testing.T) {
	newRegistry := func(operations ...*fakeBulkOperation) *bulkOperationRegistry {
		managers := map[string]*bulkOperationManager{}
		for i, operation := range operations {
			rstId := uint32(i + 1)
			managers[bulkOperationKey(rstId, "retrieve")] = &bulkOperationManager{
				clientBulkOperation: operation,
				bulkOperationEntry:  &bulkOperationEntry{RstId: rstId, Operation: "retrieve"},
			}
		}
		return &bulkOperationRegistry{managers: managers}
	}

	newRequest := func(rstId uint32, operation string) *beeremote.JobRequest {
		request := &beeremote.JobRequest{}
		request.SetRemoteStorageTarget(rstId)
		if operation != "" {
			request.SetBulkInfo(&flex.BulkJobRequestInfo{Operation: operation, JobIndex: 3})
		}
		return request
	}

	t.Run("routes to the operation matching the request's rst and operation", func(t *testing.T) {
	...
	t.Run("carries the state through to the operation", func(t *testing.T) {
		operation := &fakeBulkOperation{}
		registry := newRegistry(operation)

		require.NoError(t, registry.UpdateBulkRequest(context.Background(), newRequest(1, "retrieve"), BulkRequestFailed))

		require.Len(t, operation.updatedRequests, 1)
		assert.Equal(t, BulkRequestFailed, operation.updatedRequests[0].state)
	})

	t.Run("a request with no bulk info belongs to no operation", func(t *testing.T) {
		operation := &fakeBulkOperation{}
		registry := newRegistry(operation)
	...
	t.Run("a failure from the operation is propagated to the caller", func(t *testing.T) {
	...
	t.Run("an operation that could not be opened is reported rather than panicking", func(t *testing.T) {
		// bulkOperationManager embeds clientBulkOperation, so without the explicit wrapper this
		// would dispatch on a nil interface value.
		registry := &bulkOperationRegistry{
			managers: map[string]*bulkOperationManager{
				"1-retrieve": {bulkOperationEntry: &bulkOperationEntry{RstId: 1, Operation: "retrieve"}},
			},
		}
