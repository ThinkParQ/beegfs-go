...
func TestOverwriteMsgLen(t *testing.T) {
	...
	binary.LittleEndian.PutUint32(buf[0:4], MsgPrefix)

	err = OverwriteMsgLen(buf, 1234)
	assert.NoError(t, err)
	assert.EqualValues(t, 1234, binary.LittleEndian.Uint32(buf[4:8]))
}
...
func TestOverwriteMsgFeatureFlags(t *testing.T) {
	...
	err = OverwriteMsgFeatureFlags(buf, 1234)
	assert.NoError(t, err)
	assert.EqualValues(t, 1234, binary.LittleEndian.Uint16(buf[20:22]))
}
