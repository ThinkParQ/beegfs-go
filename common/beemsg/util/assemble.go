...
func AssembleBeeMsg(in msg.SerializableMsg) ([]byte, error) {
	...
	if err := msg.OverwriteMsgLen(plainBuf[0:msg.HeaderLen], uint32(ser.Buf.Len())+crypto.AesTagLen); err != nil {
		return nil, err
	}
	...
	iv, encrypted, err := crypto.Aes256Encrypt(plainBuf[msg.HeaderEncryptionInfoLen:])
	if err != nil {
		return nil, fmt.Errorf("Encryption failed: %w", err)
	}
	finalBuf = append(finalBuf, encrypted...)

	msg.OverwriteAesIv(finalBuf[:], iv)
...
func DisassembleBeeMsg(buf []byte, out msg.DeserializableMsg) error {
	...
	iv := msg.ExtractMsgAesIv(buf)

	// Decrypt
	err := crypto.Aes256Decrypt(iv, buf[msg.HeaderEncryptionInfoLen:])
	if err != nil {
		return err
	}
	buf = buf[:len(buf)-crypto.AesTagLen]
