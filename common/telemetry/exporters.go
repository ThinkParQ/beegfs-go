...
func buildReaders(cfg Config, log *zap.Logger) ([]sdkmetric.Reader, error) {
	var readers []sdkmetric.Reader

	if cfg.OTLP.Enabled {
		reader, err := buildOTLPReader(cfg.OTLP, log)
		if err != nil {
			return nil, err
		}
...
func buildOTLPReader(cfg OTLPConfig, log *zap.Logger) (sdkmetric.Reader, error) {
	...
	case protocolGRPC:
		if cfg.URLPath != "" {
			log.Warn("telemetry.otlp.url-path is ignored for gRPC protocol")
		}
		...
		if cfg.TLSDisable {
			...
			tlsCfg, err := buildClientTLSConfig(cfg.TLSCertFile, cfg.TLSDisableVerification, log)
			if err != nil {
...
// connects lazily. If a future SDK version adds a synchronous handshake, pass ctx
// down from New() instead.
func buildLogExporter(cfg LogsConfig, log *zap.Logger) (sdklog.Exporter, error) {
	switch cfg.Protocol {
	case protocolGRPC:
		if cfg.URLPath != "" {
			log.Warn("telemetry.logs.url-path is ignored for gRPC protocol")
		}
...
func buildClientTLSConfig(certFile string, disableVerification bool, log *zap.Logger) (*tls.Config, error) {
	...
	if disableVerification {
		log.Warn("TLS certificate verification is disabled for this OTLP connection; the connection is vulnerable to man-in-the-middle attacks")
	}
	return &tls.Config{
		RootCAs:            certPool,
