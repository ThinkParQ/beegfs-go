...
func New(cfg Config, log *zap.Logger, opts ...Option) (*Provider, error) {
	if log == nil {
		return nil, fmt.Errorf("telemetry logger is required")
	}
	p := &Provider{config: cfg, log: log}
...
func (p *Provider) initMetrics(cfg Config, res *resource.Resource, cleanupFns *[]func(context.Context) error) error {
	readers, err := buildReaders(cfg, p.log)
	if err != nil {
...
func (p *Provider) initLogs(cfg Config, res *resource.Resource, cleanupFns *[]func(context.Context) error) error {
	logExporter, err := buildLogExporter(cfg.Logs, p.log)
	if err != nil {
...
// SetLogger updates the logger used for internal warnings emitted by the Provider.
func (p *Provider) SetLogger(log *zap.Logger) {
	p.log = log
}
...
func (p *Provider) UpdateConfiguration(newConfig any) error {
	...
	if p.sdkProvider != nil || p.logProvider != nil {
		if !reflect.DeepEqual(newCfg, p.config) {
			p.log.Warn("telemetry configuration changes require a restart to take effect")
		}
	}
